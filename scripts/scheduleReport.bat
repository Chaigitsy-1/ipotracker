...
:: Create task
schtasks /create /tn "Indian_IPO_Tracker_Daily_Report" /tr "cmd.exe /c node \"%PROJECT_DIR%\scripts\sendEmailAlerts.cjs\"" /sc daily /st 06:00 /f

if %errorlevel% equ 0 (
    echo.
    echo [SUCCESS] Scheduled task registered successfully!
    echo The report and email alerts will run daily at 6:00 AM IST.
